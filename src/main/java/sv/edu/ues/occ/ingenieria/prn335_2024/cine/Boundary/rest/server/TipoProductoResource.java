...
public class TipoProductoResource implements Serializable {
    ...
    @Produces({MediaType.APPLICATION_JSON})
    @Consumes({MediaType.APPLICATION_JSON})
    public Response update(@PathParam("id") Integer id, TipoProducto tipoProducto) {
        if (id != null && tipoProducto != null && tipoProducto.getIdTipoProducto().equals(id)) {
            try {
        ...
        return Response.status(422).header("WRONG - PARAMETER", "ID:" + id + ", TipoProducto: " + tipoProducto).build();
    }
