...
public class PeliculaCaracteristicaResource implements Serializable {
    ...
    @Inject
    PeliculaCaracteristicaBean peliculaCaracteristicaBean;
    ...
    public Response findRange(
        ...
        try {
            if(firstResult >= 0 && maxResults > 0 && maxResults <= 50){
                List<PeliculaCaracteristica> encontrados = peliculaCaracteristicaBean.findRange(firstResult, maxResults);
                long total = peliculaCaracteristicaBean.Count();
    ...
    public Response findById(@PathParam("id") Long id){
        if(id != null){
            try {
                PeliculaCaracteristica encontrado = peliculaCaracteristicaBean.findById(id);
                if(encontrado != null){
    ...
    public Response create(PeliculaCaracteristica peliculaCaracteristica, @Context UriInfo uriInfo){
        if(peliculaCaracteristica != null && peliculaCaracteristica.getIdPeliculaCaracteristica() == null){
            try {
                peliculaCaracteristicaBean.Create(peliculaCaracteristica);
        ...
        return Response.status(422)
                .header("WRONG - PARAMETER", "PeliculaCaracteristica " + peliculaCaracteristica)
                .build();
    }
}
