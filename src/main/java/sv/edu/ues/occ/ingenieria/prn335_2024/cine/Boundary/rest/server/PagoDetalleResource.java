...
public class PagoDetalleResource implements Serializable {
    ...
    public Response findRange(
        ...
        try {
            if(firstResult >= 0 && maxResults > 0 && maxResults <= 50){
                List<PagoDetalle> encontrados = pagoDetalleBean.findRange(firstResult, maxResults);
                long total = pagoDetalleBean.Count();
    ...
    public Response findById(@PathParam("id") Long id){
        if(id != null){
            try {
                PagoDetalle encontrado = pagoDetalleBean.findById(id);
                if(encontrado != null){
    ...
    public Response create(PagoDetalle pagoDetalle, @Context UriInfo uriInfo){
        if(pagoDetalle != null && pagoDetalle.getIdPagoDetalle() == null){
            try {
                pagoDetalleBean.Create(pagoDetalle);
                if(pagoDetalle.getIdPagoDetalle() != null){
                    UriBuilder uriBuilder = uriInfo.getAbsolutePathBuilder();
                    uriBuilder.path(String.valueOf(pagoDetalle.getIdPagoDetalle()));
        ...
        return Response.status(422)
                .header("WRONG - PARAMETER", "PagoDetalle " + pagoDetalle)
                .build();
    }
}
